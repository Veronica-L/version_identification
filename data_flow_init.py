...
class Construct():
    def __init__(self, filename, func_addr):
        self.filename = filename
        self.func_addr = func_addr
        self.p = angr.Project(filename, load_options={'auto_load_libs': False})
        ...
        for stmt_idx, stmt in enumerate(statements):#遍历每条语句
            ...
            if isinstance(stmt, pyvex.IRStmt.WrTmp):
                ...
                if isinstance(data_get_from, pyvex.expr.RdTmp):
                    ...
                    if isinstance(data_get_from.args[1], pyvex.expr.RdTmp) and isinstance(data_get_from.args[0], pyvex.expr.Const):
                        #Add32(0x0804d760,t33)
                        rdtmp = data_get_from.args[1] #t30
                        const_int = data_get_from.args[0] #0x00000004 pyvex.expr.Const
                    ...
                    if isinstance(data_get_from.args[0], pyvex.expr.RdTmp) and isinstance(data_get_from.args[1], pyvex.expr.RdTmp):
                        # t36 = Add32(t28,t33)
                        rdtmp1 = data_get_from.args[0]
                        rdtmp2 = data_get_from.args[1]
                        print(str(rdtmp2), self.tmp_variable.keys())

                        if str(rdtmp1.tmp) in self.tmp_variable.keys():
                            #tmp有对应的寄存器名称
                            from_reg_name_1 = self.tmp_variable[str(rdtmp1.tmp)] #t30对应的寄存器名称
                        elif str(rdtmp1.tmp) in self.reg_variable.values(): 
                            #tmp没有对应的寄存器，但是它的值和某个reg一样
                            reg1_list = self.get_keys(self.reg_variable, str(rdtmp1.tmp))
                            from_reg_name_1 = reg1_list[0]
                            self.tmp_variable[str(rdtmp1.tmp)] = reg1_list[0]
                        

                        if str(rdtmp2.tmp) in self.tmp_variable.keys():
                            #tmp有对应的寄存器名称
                            print(str(rdtmp2))
                            from_reg_name_2 = self.tmp_variable[str(rdtmp2.tmp)] #t30对应的寄存器名称
                        elif str(rdtmp2.tmp) in self.reg_variable.values(): 
                            #tmp没有对应的寄存器，但是它的值和某个reg一样
                            reg2_list = self.get_keys(self.reg_variable, str(rdtmp2.tmp))
                            from_reg_name_2 = reg2_list[0]
                            self.tmp_variable[str(rdtmp2.tmp)] = reg2_list[0]

                        op_node = Op_node(from_reg_name_1, from_reg_name_2, data_get_from.op)
                        self.tmp_variable[str(data_to_tmp)] = op_node

                        # 污点tmp
                        if str(rdtmp1.tmp) in self.taint_variable_list or str(rdtmp2.tmp) in self.taint_variable_list:
                            self.taint_variable_list.append(str(data_to_tmp))
                    ...
                    if isinstance(data_get_from.addr, pyvex.expr.Const):
                        self.tmp_variable[str(data_to_tmp)] = data_get_from.addr
                    ...
                    if isinstance(data_get_from.args[0], pyvex.expr.RdTmp):
                        ...
                        if str(rdtmp.tmp) in self.taint_variable_list:
                            self.taint_variable_list.append(str(data_to_tmp))
                    if isinstance(data_get_from.args[0], pyvex.expr.Const):
                        # t29 = 32to8(0x0000002d)
                        self.tmp_variable[str(data_to_tmp)] = data_get_from.args[0].con.value
                ...
                if isinstance(data_get_from, pyvex.expr.RdTmp): #右边是tmp
                    ...
                    if isinstance(tmp_value, Mem_node):
                        # mov ebx, [ecx+edx*4]
                        # 构建关系 ecx->[ecx+edx*4], edx->[ecx+edx*4]
                        if isinstance(tmp_value.mem, Op_node):
                            arg1 = tmp_value.mem.reg_name
                            arg2 = tmp_value.mem.const_int
                            args = [arg1, arg2]
                            for arg in args:
                                if isinstance(arg, str):
                                    print('arg:', arg)
                                    print('arg_exist_nodes:', self.exists_nodes[arg])
                                    self.flow_graph.add_uniq_edge(self.exists_nodes[arg], read_node)

                        
                        #elif isinstance(tmp_value.mem, string):
                    ...
                    self.tmp_variable[str(data_get_from.tmp)] = reg_name
                ...
                print('stmt_idx',stmt_idx, len(statements))
                if stmt_idx != len(statements) - 1: continue

                print(type(stmt.guard), stmt.dst, stmt.offsIP, stmt.jk)
                if isinstance(stmt.guard, pyvex.expr.RdTmp):
                    rdtmp = stmt.guard
                    self.constraint_tmp = str(rdtmp.tmp)
                    '''
                    if str(rdtmp.tmp) in self.taint_variable_list:
                        self.end_flag = True
                        break
                    if isinstance(self.tmp_variable[str(rdtmp.tmp)], Op_node):
                        arg1 = self.tmp_variable[str(rdtmp.tmp)].reg_name
                        arg2 = self.tmp_variable[str(rdtmp.tmp)].const_int
                        if arg1 in self.exists_nodes.keys():
                            if self.exists_nodes[arg1] in self.taint_list
                    else:
                        # 不要跳转
                        self.jump = False
                    '''
    ...
    def analysis_taint(self): # node_and_edge:[node, edge]
            ...
            if isinstance(node_name, Mem_node):   
                if isinstance(node_name.mem, Op_node):
                    ...
                    if node_name.mem.reg_name == 'ebp' and node_name.mem.const_int == 12: #and node_name.mem.opration == 'Iop_And32': #[ebp+argv]
                        #if read_or_write == 'r' and addr == '0x804b2bc': #fuser
                        #if read_or_write == 'r' and addr == '0x804d4c2': #netstat
                        #if read_or_write == 'r' and addr == '0x8049618': #cat
                        #if read_or_write == 'r' and addr == '0x804988f': #chown
                        if read_or_write == 'r' and addr == '0x8049790': #dnsdomainname
        ...
        print('taint_list:',self.taint_list)
    ...
    def if_enter_call(self): 
        ...
        for mem_node in self.mem_store.keys():
            if isinstance(mem_node, pyvex.expr.Const): continue
    ...
    def get_data_flow(self):
        ...
        addr_to_node_dict = {}
        for c in self.function_cfg.nodes():
            addr_to_node_dict[c.addr] = c
        for c in self.function_cfg.nodes():
            cfg_node = c
            break
        ...
        while self.end_flag == False:
            ...
            # 对最后的constraint_tmp进行判断
            if self.constraint_tmp in self.taint_variable_list:
                self.end_flag = True
            
            elif self.constraint_tmp != '':
                print('constraint_tmp', self.constraint_tmp)
                tmp_node = self.tmp_variable[self.constraint_tmp]
                if isinstance(tmp_node, Op_node):
                    arg1 = tmp_node.reg_name
                    arg2 = tmp_node.const_int
                    print('arg:')
                    args = [arg1, arg2]
                    for arg in args:
                        if isinstance(arg, Mem_node):
                            arg = arg.mem
                        print(arg)
                        if arg in self.exists_nodes.keys():
                            node_arg = self.get_keys(nodes, self.exists_nodes[arg])[0]
                            print(node_arg)
                            if node_arg in self.taint_list:
                                self.end_flag = True                    
                else:
                # 不要跳转
                    self.jump = False
            ...
            if irsb.jumpkind == 'Ijk_Call':
                ...
                if self.if_enter_call(): 
                    ...
                    self.taint_eax = True
                    # 将eax加入taint node
                    self.eax_node = self.create_node('eax', src_addr+irsb.size, 'w')
                    print('call_node', self.eax_node)
                ...
                #分析完对[esp]弹出
                self.clear_stack()
                cfg_node = addr_to_node_dict[src_addr+irsb.size]
            elif irsb.jumpkind == 'Ijk_Boring':
                if irsb.exit_statements != () and self.jump == False:
                    cfg_node = addr_to_node_dict[src_addr+irsb.size]
                else:
                    cfg_node = addr_to_node_dict[irsb.next.con.value]
        ...
        for cfg_node in self.function_cfg.nodes():
            ...
            if irsb.jumpkind == 'Ijk_Call':
                ...
                if self.if_enter_call(): 
                    ...
                    self.taint_eax = True
                    # 将eax加入taint node
                    self.eax_node = self.create_node('eax', src_addr+5, 'w')
                    self.flow_graph.add_node(self.eax_node)
                else:
                    self.not_track_call.append(irsb.next)
                    print('not_track_call:', irsb.next)
...
'''
def main():
    #func_addr = 0x804b1a0 #fuser
    #func_addr = 0x804d4b0 #netstat
    #func_addr = 0x80495b0 #cat
    #func_addr = 0x8049880 #chown
    func_addr = 0x8049700  #dnsdomainname
    target_addrs = [134516818]
    filename = "dnsdomainname"
    c = Construct(filename, func_addr)
    taint_node_list = c.get_data_flow()
    print(taint_node_list)
